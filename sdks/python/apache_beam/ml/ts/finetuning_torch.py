...
class TimesFMFinetuner:
  ...
  # EDITED TO INLCUDE DIFFERENT HORIZON LENGHTS != 128
  def _process_batch(self, batch: List[torch.Tensor]) -> tuple:
    """Process a single batch of data.
    ...
    horizon_len = x_future.shape[1] 

    predictions = self.model(x_context, x_padding.float(), freq)
    predictions_mean = predictions[..., 0]
    last_patch_pred = predictions_mean[:, -1, :]

    sliced_pred = last_patch_pred[:, :horizon_len] # Shape becomes [batch_size, horizon_len]

    loss = self.loss_fn(sliced_pred, x_future.squeeze(-1))
    if self.config.use_quantile_loss:
      quantiles = self.config.quantiles or _create_quantiles()
      for i, quantile in enumerate(quantiles):
        last_patch_quantile = predictions[:, -1, :, i + 1]
        
        # Also slice the quantile prediction to match the horizon
        sliced_quantile_pred = last_patch_quantile[:, :horizon_len] 

        loss += torch.mean(
            self._quantile_loss(sliced_quantile_pred, x_future.squeeze(-1),
                                quantile))
